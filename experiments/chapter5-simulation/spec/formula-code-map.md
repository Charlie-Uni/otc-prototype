...
| F-SPILL-3 | Next-period spillover demand combination | `runner/network-demand.ts::applyIncomingSpilloverToDecision` | base Logistic probability and previous-tick fund spillover bps | probability capped at 1, same draw reused | `runner/network-demand.test.ts`; `runner/run.test.ts` |

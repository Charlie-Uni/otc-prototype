...
test('accepts the mentor-approved pilot baseline', () => {
  ...
  assert.equal(config.control.baselinePhiBps, 10_000);
...
test('rejects unordered control scans and baselines outside their scan', () => {
  const unordered = structuredClone(baseline) as Record<string, Record<string, unknown>>;
  unordered.control.phiScanBps = [0, 5_000, 2_500, 10_000];
  assert.throws(() => parseSimulationConfig(unordered), /CONTROL_PHI_VALUES_NOT_ASCENDING/);

  const missing = structuredClone(baseline) as Record<string, Record<string, unknown>>;
  missing.control.releaseConsecutiveTicks = 2;
  assert.throws(

...
- Foundry：70 项合约测试通过；六指标评分、NAV、认购与赎回四组 Fuzz 各运行 256 组输入。
- API：69 项 Node/TypeScript 测试通过。
- 静态检查：TypeScript `--noEmit` 通过。
- 端到端：初始 NAV、现金认购换份额、净资产计算 NAV、赎回现金金额、链上指标溯源、1883/7702 bps 评分、30 天陈旧警告、Gate、R0-R4、审计与仿真全部通过。
- 合约行覆盖率：FundToken 97.81%、NAVRegistry 100%、RiskRegistry 97.22%，业务合约合计 97.92%；原始输出由统一测试入口生成到 `coverage.log`。部署脚本由端到端 smoke 验证，不计入业务合约覆盖率。
- 运行时代码：FundToken 21671 B、NAVRegistry 9851 B、RiskRegistry 19095 B，均低于 EIP-170 的 24576 B；最小余量为 FundToken 的 2905 B。
- 本地最新完整证据为 `test-results/chapter3/20260728T105332Z`，除未配置 PostgreSQL 外全部通过。`docs/evidence/chapter3-summary.json` 仍是上一已提交版本的固定 CI 摘要；本轮 ABI 和业务变更合并后必须由启用 PostgreSQL 16 的新 CI 重新锚定，不能将旧摘要作为当前版本证据。

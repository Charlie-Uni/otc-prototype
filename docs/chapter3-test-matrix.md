...
- API：本开发周期当前 99 项 Node/TypeScript 测试通过；最终数量以 v1.4.0 重锚定摘要为准。新增覆盖 SubscriptionFlow、SettlementDelay、共享闭区间窗口、receipt 多日志精确确认以及 PostgreSQL/内存存储分离；payload v3 golden hash 保持不变。
